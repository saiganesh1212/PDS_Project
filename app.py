...
class Users(db.Model):
    ...
    role_id = db.Column(db.Integer, db.ForeignKey('roles.roleid'), nullable=False)
...
def register():
    if request.method == 'POST':
        ...
        new_user_cred = UserCredentials(username=username, password_hash=hashed_password)
        db.session.add(new_user_cred)
        db.session.commit()

        #logic for creating a new user record

        new_user=Users(name=username,gender='',age=0,emergencycontactno='',createddate=datetime.utcnow(),role_id=2)
...
def profile():
    try:
        userdetails=Users.query.filter_by(id=request.get_json()['id']).first()
        genders=['Male','Female']
...
def visualizeHeartRate():
    ...
    selectedUserHeartData=HeartRate.query.filter_by(userid=selectedUserId).all()
    userheartRate=[data.heartbeatvalue for data in selectedUserHeartData]
    
    heartBeatRange=[random.randint(40,120) for i in range(len(userheartRate))]
    print(userheartRate)
    print(heartBeatRange)
